...
#SBATCH --mem=400G
#SBATCH --time=6:00:00
#SBATCH --output=/scratch/m000081-pm06/eprakash/logs/sup_jepa_aux_%j.out
#SBATCH --error=/scratch/m000081-pm06/eprakash/logs/sup_jepa_aux_%j.err

# ============================================================
# Supervised pair-head CE + W_JEPA=0.1 dynamic-sentence JEPA aux.
# Gold = finding head only (same as 0.457). Does not overwrite
# checkpoints_supervised_progression_unfrozen/.
#
#   sbatch train_supervised_jepa_aux.sh
# ============================================================
...
torchrun --nproc_per_node=4 train_supervised_jepa_aux.py --train \
    ...
    --jepa-weight 0.1 \
    --num-workers 4 \
    --out-dir checkpoints_supervised_progression_jepaaux \
    --image-root "mimic=$JEPA_IMAGE_ROOTS_DIR/mimic" \
    --image-root "chexpert=$JEPA_IMAGE_ROOTS_DIR/chexpert/train" \
    --image-root "rexgradient=$JEPA_IMAGE_ROOTS_DIR/rexgradient/deid_png" \
    "$@"
