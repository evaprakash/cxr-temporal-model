...
if local_rank == 0:
    print(
        f"[train] local text_projection init=official_cls "
        f"(dense_to_hidden {tuple(_w.shape)} ||W||={_w.norm().item():.4f} "
        f"match={bool(torch.equal(_w, _w_cls))} "
        f"text_trainable={_txt_trainable})"
    )
    if not _txt_trainable:
        raise RuntimeError(
            "text encoder has no trainable params; this run inits "
            "from official CLS then trains"
        )
if FREEZE_TEXT_ENCODER:
